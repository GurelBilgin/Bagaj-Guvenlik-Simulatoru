...
* İstenirse tek TXT rapor dosyası kaydetme
...
7. İstenirse `TXT Rapor Kaydet` butonu ile tek rapor dosyası oluşturulabilir.
...
```text
v1.0.2 - Tek Rapor Dosyası Güncellemesi
```

Bu sürümde TXT rapor kaydetme sistemi sadeleştirilmiştir. Artık rapor kaydedildiğinde yalnızca tarih-saat bilgisi içeren tek bir TXT dosyası oluşturulur.

Önceki sürümlerde oluşturulan `son_gun_sonu_raporu.txt` adlı ikinci kopya kaldırılmıştır.
...
Rapor kaydedildiğinde bir TXT dosyası oluşturulur:

```text
gun_sonu_raporu_YYYYMMDD_HHMMSS.txt
```

Örnek:

```text
gun_sonu_raporu_20260612_190734.txt
```

### 📂 Rapor Dosyalarının Kaydedildiği Yer

Kaynak koddan çalıştırıldığında raporlar proje klasöründeki `reports/` klasörüne kaydedilir:

```text
Bagaj-Guvenlik-Simulatoru/reports/
```

EXE üzerinden çalıştırıldığında raporlar EXE dosyasının bulunduğu dizindeki `reports/` klasörüne kaydedilir:

```text
BagajGuvenlikSimulatoru.exe
reports/
```

Örneğin EXE dosyası `Downloads` klasöründeyse raporlar şu klasörde oluşur:

```text
Downloads/reports/
```

Uygulama içindeki `Rapor Klasörünü Aç` butonu ile bu klasör doğrudan açılabilir.
...
* EXE dosyası GitHub deposuna doğrudan eklenmemiştir; GitHub Releases üzerinden paylaşılmıştır.
