...
class BagajGuvenlikApp(tk.Tk):
    ...
    def __init__(self) -> None:
        ...
        # PyInstaller ile tek dosya EXE çalıştırıldığında iki farklı dizin gerekir:
        # - proje_dizini: EXE içine gömülen assets/icon/ses gibi kaynakları okumak için.
        # - calisma_dizini: kullanıcı çıktıları, özellikle raporlar için kalıcı klasör.
        self.proje_dizini, self.calisma_dizini = self._dizinleri_belirle()
        self.reports_dir = self.calisma_dizini / "reports"
    ...
    def _dizinleri_belirle(self) -> tuple[Path, Path]:
        """Kaynak ve çalışma dizinlerini Python/EXE durumuna göre belirler.

        Normal Python çalıştırmada iki dizin de proje klasörüdür.

        PyInstaller onefile EXE çalıştırmada ise:
        - sys._MEIPASS geçici açılan klasördür; assets gibi gömülü kaynaklar burada okunur.
        - sys.executable dosyasının bulunduğu klasör kalıcıdır; TXT raporlar burada
          oluşturulan reports klasörüne kaydedilir.
        """
        if getattr(sys, "frozen", False):
            kaynak_dizini = Path(getattr(sys, "_MEIPASS", Path(sys.executable).resolve().parent))
            calisma_dizini = Path(sys.executable).resolve().parent
        else:
            kaynak_dizini = Path(__file__).resolve().parent
            calisma_dizini = kaynak_dizini

        return kaynak_dizini, calisma_dizini
    ...
    def _xray_sahnesini_ciz(self) -> None:
        ...
        self.xray_canvas.create_text(
            (self.detector_x1 + self.detector_x2) / 2,
            self.detector_y1 + 10,
            text="DEDEKTÖR",
            fill="#bfdbfe",
            font=("Arial", 10, "bold"),
        ...
        self.xray_canvas.create_line(
            self.entry_text_x + 35,
            92,
            self.detector_x1 - 24,
            92,
            fill="#38bdf8",
    ...
    def _bagaj_ciz(self, x: int, y: int, result: ScanResult) -> None:
        ...
        self._bagaj_items.append(
            self.xray_canvas.create_rectangle(x, y, x + 84, y + 48, fill=fill, outline=outline, width=3)
        )
        self._bagaj_items.append(
            self.xray_canvas.create_rectangle(x + 27, y - 12, x + 57, y + 2, fill="#0f172a", outline=outline, width=2)
        )
        ...
        self._bagaj_items.append(
            self.xray_canvas.create_text(x + 42, y + 29, text=etiket, fill="#f8fafc", font=("Arial", 8, "bold"))
        )
    ...
    def _sonuc_rozetini_goster(self, result: ScanResult) -> None:
        ...
        self._dinamik_canvas_items.append(
            self.xray_canvas.create_rectangle(x1, y1, x2, y2, fill=dolgu, outline=renk, width=3)
        )
        self._dinamik_canvas_items.append(
            self.xray_canvas.create_text((x1 + x2) / 2, y1 + 20, text=metin, fill=renk, font=("Arial", 14, "bold"))
        )
        self._dinamik_canvas_items.append(
            self.xray_canvas.create_text((x1 + x2) / 2, y1 + 42, text=detay[:22], fill="#f8fafc", font=("Arial", 8, "bold"))
        )
    ...
    def rapor_penceresi_goster(self) -> None:
        ...
        ttk.Label(
            kapsayici,
            text="Uygulama içi özet ekranı — TXT dosyası açmadan raporu görüntüler.",
            style="Sub.TLabel",
    ...
    def _rapor_karti(self, parent: ttk.Frame, col: int, baslik: str, deger: str, renk: str) -> None:
        ...
        tk.Label(kart, text=baslik, bg="#ffffff", fg="#4b5563", font=("Arial", 9, "bold")).pack(
            anchor="w",
            padx=10,
            pady=(8, 0),
        )
        tk.Label(kart, text=deger, bg="#ffffff", fg=renk, font=("Arial", 20, "bold")).pack(
            anchor="w",
            padx=10,
            pady=(0, 8),
        )
    ...
    def rapor_olustur(self) -> None:
        # Raporlar kalıcı olarak çalışma dizinindeki reports klasörüne kaydedilir.
        # EXE üzerinden çalıştırıldığında bu klasör EXE'nin yanında oluşur.
